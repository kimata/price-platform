...
# Global liveness manager instance
_liveness_manager: LivenessManager | None = None


def get_liveness_manager() -> LivenessManager | None:
    """Get the global liveness manager instance.

    Returns:
        The liveness manager instance, or None if not initialized.
    """
    return _liveness_manager


def set_liveness_manager(manager: LivenessManager | None) -> None:
    """Set the global liveness manager instance.

    Args:
        manager: The liveness manager to set, or None to clear.
    """
...
def init_liveness_manager(
    ...
    *,
    ...
    """Initialize and return a new liveness manager.

    Args:
        liveness_file: Path to liveness file (None to disable updates).
        update_interval_sec: Interval for liveness updates.
        update_fn: Callable to update the liveness file.

    Returns:
        The newly created liveness manager.
    """
...
def _reset_liveness_manager() -> None:
    """Reset liveness manager for testing."""
    global _liveness_manager
    _liveness_manager = None
