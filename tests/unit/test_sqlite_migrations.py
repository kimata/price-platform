...
def test_price_event_store_adds_selection_column_once(tmp_path: pathlib.Path) -> None:
    db_path = tmp_path / "events.db"
    schema_dir = tmp_path / "schema"
    schema_dir.mkdir()
    _write_schema(
        schema_dir / "sqlite_price_events.schema",
        """
    ...
    assert "color_key" in columns
    assert "ensure-selection-column:color_key" in migrations
...
def test_webpush_store_renames_legacy_columns(tmp_path: pathlib.Path) -> None:
    ...
    schema_dir.mkdir()
    _write_schema(
        schema_dir / "sqlite_webpush.schema",
        """
    ...
    BaseWebPushStore(
        db_path=db_path,
        schema_dir=schema_dir,
        group_filter_column="group_filter",
        legacy_group_filter_columns=("maker_filter",),
    ...
    assert "migrate-webpush-columns" in migrations
